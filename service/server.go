...
func NewServer(conf *config.Config, userClient v1.UserServiceClient) (v1.UserServiceServer, error) {
...
func (s *Server) InviteUser(ctx context.Context, rq *v1.InviteUserRequest) (*v1.InviteUserResponse, error) {
	newCtx, cancel := context.WithCancel(ctx) // cancel
	defer cancel()

	if err := s.datastore.Users().Create(newCtx, rq); err != nil {
		return &v1.InviteUserResponse{Msg: "邀请用户失败", Status: v1.Status_failure}, err
	}

	// 调用一下notification client ,失败了上面cancel掉

	return &v1.InviteUserResponse{Msg: "邀请用户成功", Status: v1.Status_success}, nil
